...
describe("Cache Service", () => {
  it("should extract domain correctly", () => {
    expect(cacheService.getDomain("https://google.com/search?q=test")).toBe("google.com");
    expect(cacheService.getDomain("https://sub.domain.co.uk/page")).toBe("sub.domain.co.uk");
  });

  it("should generate correct relative path", () => {
    const url = "https://example.com";
    const path = cacheService.getPath(url, 200);
